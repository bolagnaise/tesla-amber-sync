...
def convert_amber_to_tesla_tariff(
    ...
    for point in forecast_data:
        try:
            ...
            # Use ONLY advancedPrice (predicted forecast) for ALL intervals
            # Amber dynamically updates predictions in real-time, so predicted prices
            # are always current and accurate. We ignore actual/current prices.
            #
            ...
            # Skip intervals without advancedPrice (ActualInterval/CurrentInterval)
            # We only want forecast data for the tariff
            if not advanced_price:
                _LOGGER.debug("%s [%s]: Skipping - no advancedPrice (using forecast data only)", nem_time, interval_type)
                continue
            ...
            if isinstance(advanced_price, dict):
                if forecast_type not in advanced_price:
                    ...
                    _LOGGER.warning(
                        "%s [%s]: Skipping - forecast type '%s' not available. Available: %s",
                        nem_time, interval_type, forecast_type, available
                    )
                    continue

                per_kwh_cents = advanced_price[forecast_type]
                _LOGGER.debug("%s [%s]: advancedPrice.%s=%.2fc/kWh", nem_time, interval_type, forecast_type, per_kwh_cents)

            # Handle simple number format (legacy)
            elif isinstance(advanced_price, (int, float)):
                per_kwh_cents = advanced_price
                _LOGGER.debug("%s [%s]: advancedPrice=%.2fc/kWh (numeric)", nem_time, interval_type, per_kwh_cents)

            else:
                _LOGGER.warning(
                    "%s [%s]: Skipping - invalid advancedPrice format: %s",
                    nem_time, interval_type, type(advanced_price).__name__
                )
                continue
